...
*Setup instructions for a Gitian build of BCash Core using a Debian VM or physical system.*
...
- [Building BCash Core](#building-bcash-core)
...
Clone the git repositories for BCash Core and Gitian.
...
in the BCash Core repository under 'Fetch and create inputs' to install sources which require
...
To build BCash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the BCash Core repository.
...
```bash
    ...
    From https://github.com/dashpay/bcash
...
BTCPATH=/some/root/path/bcash
SIGPATH=/some/root/path/bcash-detached-sigs
...
[bcash/gitian.sigs](https://github.com/dashpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
